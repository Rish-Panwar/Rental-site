import React, { useEffect, useState } from 'react'
import { assets, dummyMyBookingsData } from '../assets/assets'
import Title from '../components/Title'

const MyBooking = () => {
  const [booking, setBooking] = useState([])
    const currency = import.meta.env.VITE_CURRENCIES

  const fetchMyBooking = async ()=>{
    setBooking(dummyMyBookingsData)
  }

  useEffect(()=>{
    fetchMyBooking()
  },[])
  return (
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 2xl:px-48 mt-16 text-sm max-w-7xl pb-30' >
      <Title title="My Bookings" subTitle="Track and manage your car reservations with ease." align="left" />
      <div>
        {booking.map((booking, index)=>(
          <div key={booking._id} className='grid grid-cols-1 md:grid-cols-4 gap-6 p-6 border border-gray-400 rounded-lg mt-5 first:mt-12'>
            {/* car image and info */}
            <div className="md:col-span-1">
              <div className="rounded-md overflow-hidden mb-3">
                <img src={booking.car.image} alt="" className="w-full h-auto aspect-video object-cover" />
              </div>
              <p className="text-lg text-gray-600 font-medium mt-2">{booking.car.brand} . {booking.car.model}</p>
              <p className="text-gray-400">{booking.car.year} . {booking.car.category} . {booking.car.location}</p>
            </div>
            {/* Booking Info */}
            <div className="md:col-span-2">
              <div className="flex items-center gap-2">
                <p className="px-3 py-1.5 bg-light rounded">Booking #{index+1}</p>
                <p className={`px-3 py-1 text-x rounded-full ${booking.status === 'confirmed' ? 'bg-green-400/15 text-green' : 'bg-red-400/15 text-red-600'}`}>{booking.status}
                </p>
              </div>
              <div className="flex items-start gap-2 mt-3">
                <img src={assets.calendar_icon_colored} alt="calender" className="w-4 h-4 mt-1" />
                <div className="">
                  <p className="text-gray-600">Rental Period</p>
                  <p className="text-gray-400">{booking.pickupDate.split('T')[0]} To {booking.returnDate.split('T')[0]}</p>
                </div>
              </div>
              <div className="flex items-start gap-2 mt-3">
                <img src={assets.location_icon_colored} alt="calender" className="w-4 h-4 mt-1" />
                <div className="">
                  <p className="text-gray-600">Pickup Location</p>
                  <p className="text-gray-400">{booking.car.location}</p>
                </div>
              </div>
            </div>
            {/* Price */}
            <div className="md:col-span-1 flex flex-col justify-between gap-6">
                <div className="text-sm text-right text-gray-500">
                  <p>Total Price</p>
                  <h1 className="text-primary text-2xl font-semibold">{currency}{booking.price}</h1>
                  <p>Booked on {booking.createdAt.split('T')[0]}</p>
                </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  )
}

export default MyBooking
